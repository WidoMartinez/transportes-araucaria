...
router.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error al obtener configuración de pasarelas" });
  }
});
...
router.put("/", apiLimiter, authJWT, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error al actualizar configuración de pasarelas" });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const oldPublicId = extractCloudinaryPublicId(configFinal[gateway]?.imagen_url);
      if (oldPublicId) {
        try {
          await cloudinary.uploader.destroy(oldPublicId);
          console.log(`🗑️ [PASARELA] Imagen anterior eliminada: ${oldPublicId}`);
        } catch (e) {
          console.error("Error eliminando imagen anterior de Cloudinary:", e.message);
        }
      }

      // Subir nueva imagen a Cloudinary
      const cloudResult = await subirImagenPasarela(req.file.buffer, req.file.mimetype, gateway);
      console.log(`🖼️ [PASARELA] Imagen subida a Cloudinary: ${cloudResult.public_id}`);
      ...
      res.status(500).json({ error: "Error al subir la imagen. Intenta nuevamente." });
    }
  },
);
...
router.delete("/:gateway/imagen", apiLimiter, authJWT, async (req, res) => {
  ...
  try {
    ...
    const publicId = extractCloudinaryPublicId(configFinal[gateway]?.imagen_url);
    if (publicId) {

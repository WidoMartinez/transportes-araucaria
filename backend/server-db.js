...
// --- ENDPOINT ADMIN: Obtener link de evaluación manual (sin enviar correo) ---
// AVISO: Este endpoint se despliega en Render.com. No modificar infraestructura sin aprobación.
app.get("/api/admin/evaluaciones/link/:reservaId", authJWT, apiLimiter, async (req, res) => {
	try {
		const reservaId = Number(req.params.reservaId);
		if (!Number.isInteger(reservaId) || reservaId <= 0) {
			return res.status(400).json({ success: false, error: "reservaId inválido" });
		...
		// Buscar registro de evaluación existente
		let evaluacion = await EvaluacionConductor.findOne({ where: { reservaId } });

		// Si ya fue evaluada, no tiene sentido enviar el link
		if (evaluacion?.evaluada) {
			return res.status(400).json({ success: false, error: "Esta reserva ya fue evaluada por el pasajero" });
		}

		const ahora = new Date();
		const tokenVencido = evaluacion?.tokenExpiracion && ahora > new Date(evaluacion.tokenExpiracion);

		// Crear o regenerar token si no existe o está vencido
		if (!evaluacion || tokenVencido) {
			const nuevoToken = generarTokenEvaluacion();
			const nuevaExpiracion = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);

			if (!evaluacion) {
				// Crear registro nuevo (sin marcar solicitudEnviada para no confundir con el flujo de correo)
				if (!reserva.email) {
					return res.status(400).json({ success: false, error: "La reserva no tiene email del pasajero" });
				}
				evaluacion = await EvaluacionConductor.create({
		...
		// Construir URL del frontend (sin barra al final)
		const frontendBase = (process.env.FRONTEND_URL || "https://www.transportesaraucaria.cl").replace(/\/$/, "");
		const link = `${frontendBase}/#evaluar?token=${evaluacion.tokenEvaluacion}`;

		console.log(`🔗 [Evaluación] Link manual generado para reserva ${reservaId}`);

		return res.json({
			success: true,
			link,
			tokenExpiracion: evaluacion.tokenExpiracion,
			clienteNombre: evaluacion.clienteNombre,
			clienteEmail: evaluacion.clienteEmail,
		});
	} catch (error) {
		console.error("❌ [admin/evaluaciones/link] Error:", error.message);
		return res.status(500).json({ success: false, error: "Error interno al obtener link de evaluación" });
	}
});

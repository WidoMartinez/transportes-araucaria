...
async function testPricing() {
	try {
		...
		const response = await fetch('https://transportes-araucaria.onrender.com/pricing');
		...
		data.dayPromotions?.forEach((promo, index) => {
			if (!Array.isArray(promo.dias)) {
				console.error(`❌ Promoción ${index}: dias no es array:`, typeof promo.dias, promo.dias);
			}
		});

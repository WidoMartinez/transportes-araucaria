...
function App() {
	...
	useEffect(() => {
        ...
        const handlePayment = async (gateway, type = "abono") => {
                ...
                const description =
                        type === "total"
                                ? `Pago total con descuento para ${destinoFinal} (${vehiculo || "Vehículo a confirmar"})`
                                : `Abono reserva (40%) para ${destinoFinal} (${vehiculo || "Vehículo a confirmar"})`;
	...
	const maxPasajeros = useMemo(() => {
        ...
        const { precioBase, descuentoOnline, totalConDescuento, abono, saldoPendiente } = pricing;
        ...
        if (formData.equipajeEspecial) {
                extrasList.push({ label: "Equipaje", value: formData.equipajeEspecial, fullWidth: true });
        }
        if (formData.mensaje) {
                extrasList.push({ label: "Notas", value: formData.mensaje, fullWidth: true });
        }

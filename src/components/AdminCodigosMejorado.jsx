...
export default function AdminCodigosMejorado() {
  ...
  const cargarCodigos = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/codigos/buscar?${params}`);
  ...
  const cargarUsuariosCodigo = async (codigoId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/codigos/${codigoId}/usuarios`);
  ...
  const eliminarUsuario = async (codigoId, usuarioId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/codigos/${codigoId}/usuarios/${usuarioId}`, {
        method: 'DELETE'
      });
  ...
  const resetearCodigo = async (codigoId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/codigos/${codigoId}/reset`, {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => { cargarCodigos(); cargarEstadisticas(); }}>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{estadisticas.totalCodigos}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{estadisticas.codigosActivos}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{estadisticas.codigosAgotados}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setFiltros(prev => ({ ...prev, agotado: value }))}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Todos" />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setFiltros(prev => ({ ...prev, vencido: value }))}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Todos" />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setFiltros(prev => ({ ...prev, ordenar: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setFiltros(prev => ({ ...prev, direccion: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                ...
                <TableBody>
                  {codigos.map((codigo) => (
                    <TableRow key={codigo.id}>
                      ...
                      <TableCell>{formatearFecha(codigo.fechaVencimiento)}</TableCell>
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                <AlertDialogTitle>¿Resetear código?</AlertDialogTitle>
                                <AlertDialogDescription>
                                  Esta acción eliminará todos los usuarios y reseteará el contador de usos del código "{codigo.codigo}".
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
              <div>
                ...
                <p className="text-lg font-bold">{codigoSeleccionado?.usosActuales}</p>
              </div>
              <div>
                <p className="text-sm font-medium">Límite de Usos</p>
                <p className="text-lg font-bold">{codigoSeleccionado?.limiteUsos}</p>
              </div>
              <div>
              ...
              <TableBody>
                {usuariosCodigo.map((usuario, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {usuario.fechaUso ? formatearFecha(usuario.fechaUso) : 'N/A'}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex space-x-1">
                        <Badge variant={usuario.estado === 'confirmada' ? 'default' : 'secondary'}>
                          {usuario.estado}
                        </Badge>
                        <Badge variant={usuario.estadoPago === 'pagado' ? 'default' : 'outline'}>
                          {usuario.estadoPago}
                        </Badge>
                      </div>
                    </TableCell>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Esta acción eliminará a {usuario.nombre} ({usuario.email}) del código.
                              El contador de usos se reducirá en 1.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancelar</AlertDialogCancel>
                            <AlertDialogAction
                              onClick={() => eliminarUsuario(codigoSeleccionado.id, usuario.usuarioId)}
                            >
                              Eliminar
                            </AlertDialogAction>
                          </AlertDialogFooter>

...
function HeroExpress({
	...
	const [horaTerminoServicioActivo, setHoraTerminoServicioActivo] =
		useState(null);
		...
		if (currentStep === 1) {
			const messages = [
				...
				{ icon: <Users className="h-3 w-3" />, text: "Alta demanda para hoy" },
				{
					...
					text: `${Math.floor(Math.random() * 3) + 2} personas cotizando`,
				},
			];
	...
	const timeOptions = useMemo(() => {
		...
		if (
			...
			oportunidadesRetornoUniversal.opciones
		) {
	...
	const richInfo = useMemo(() => {
		...
		if (formData.destino && formData.destino !== "Aeropuerto La Araucanía") {
			return {
				title: `Viaja a ${formData.destino}`,
				subtitle: "Comodidad y seguridad garantizada.",
			};
		}
		if (formData.origen && formData.origen !== "Aeropuerto La Araucanía") {
			return {
				title: `Viaja desde ${formData.origen}`,
				subtitle: "Comenzamos el viaje donde tú estés.",
			};
		}
		...
		return {
			...
			beneficios: [
				{
					text: "Tarifas fijas y transparentes",
					icon: ShieldCheck,
					color: "text-emerald-400",
				},
				{
					text: "Reserva inmediata 100% online",
					icon: Zap,
					color: "text-amber-400",
				},
				{
					text: "Sillas para niños obligatorias",
					icon: Baby,
					color: "text-blue-400",
				},
				{
					text: "Paga en hasta 3 cuotas sin intereses",
					icon: CreditCard,
					color: "text-emerald-400",
				},
			],
	...
	useEffect(() => {
		...
		const descuento = calcularDescuentoEscalonado(
	...
	const handleStepOneNext = async () => {
		...
		if (fechaSeleccionada < hoy)
			return setStepError("La fecha no puede ser pasada.");

		if (formData.idaVuelta) {
			if (!formData.fechaRegreso)
				return setStepError("Selecciona la fecha de regreso.");
			if (!formData.horaRegreso)
				return setStepError("Selecciona la hora de regreso.");
			const fechaRegreso = new Date(`${formData.fechaRegreso}T00:00:00`);
			if (fechaRegreso < fechaSeleccionada)
				return setStepError(
					"La fecha de regreso no puede ser anterior a la ida.",
				);
		}
		...
		try {
			...
			if (respBloqueo.ok) {
				...
				if (dataBloqueo.bloqueado) {
					setStepError(
						`Agenda completada. ${dataBloqueo.motivo || "Esta fecha/hora no está disponible para reservas."}`,
					);
					return;
				}
			}
		...
		if (formData.destino !== "Otro") {
			...
			if (destinoObj) {
				...
				if (diffHoras < anticipacion) {
					setStepError(
						`Para ${destinoObj.nombre}, reserva con al menos ${anticipacion} horas de anticipación.`,
					);
					return;
				}
			}
	...
	const validarDatosReserva = () => {
		...
		if (!emailRegex.test(formData.email))
			return setStepError("Email inválido.");
	...
	return (
		<section
			id="inicio"
			className="relative w-full min-h-screen flex flex-col lg:grid lg:grid-cols-2 bg-transparent pt-20"
		>
					...
					<AnimatePresence mode="wait">
						{richInfo.isSummary ? (
							...
							>
								...
								<p
									className="text-sm text-white font-medium mb-2 line-clamp-2 max-w-[85%]"
								...
								{richInfo.isRich && (
									<div className="flex flex-wrap justify-center gap-2 mt-2">
										<Badge
											variant="secondary"
											className="bg-white/90 backdrop-blur-sm text-xs font-semibold px-2.5 py-1 h-7 flex items-center gap-1.5 text-primary border-0 shadow-sm"
										>
			...
			<div className="relative flex flex-col justify-start lg:justify-center px-4 sm:px-6 py-6 sm:py-8 lg:p-16 xl:p-24 overflow-y-auto bg-transparent z-10 -mt-6 mx-4 sm:mx-6 lg:mx-0 rounded-t-3xl rounded-b-3xl lg:mt-0 lg:rounded-none shadow-none mb-6 lg:mb-0">
				{/* Indicador de pasos — visible en móvil, oculto en desktop */}
				<div className="flex items-center justify-center gap-2 mb-4 lg:hidden">
					<div
						className={`h-2 rounded-full transition-all duration-300 ${currentStep === 0 ? "w-6 bg-primary" : "w-2 bg-primary/30"}`}
					/>
					<div
						className={`h-2 rounded-full transition-all duration-300 ${currentStep === 1 ? "w-6 bg-primary" : "w-2 bg-primary/30"}`}
					/>
				</div>
								...
								>
									...
									>
										...
										<span className="text-sm font-medium text-chocolate-800">
											¡Ahorra hasta un 50%! Revisa nuestras{" "}
											<span className="font-bold underline decoration-chocolate-400 group-hover:decoration-chocolate-600">
												tarifas bajas en Oportunidades
											</span>
										</span>
									</a>
								</motion.div>
							...
							<div className="space-y-5 md:space-y-4 bg-white/80 backdrop-blur-md p-6 sm:p-8 rounded-3xl lg:rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100/50">
								...
								<div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 md:gap-3 md:items-end">
									<div className="space-y-2">
										<Label
											htmlFor="origen"
											className="text-base md:text-sm font-semibold text-foreground"
										>
											Origen
										</Label>
									...
									<div className="space-y-2">
										...
										<div className="relative">
											...
											<select
												...
												onChange={(e) => {
													handleInputChange({
														target: { name: "hora", value: e.target.value },
													});
												}}
								...
								{descuentoEscalonadoInfo && reservaActiva && (
									<AlertaDescuentoRetorno
										...
										onSeleccionarHorario={(horaSeleccionada) => {
											...
											const horaStr = horaSeleccionada.toLocaleTimeString(
												"es-CL",
												{
													hour: "2-digit",
													minute: "2-digit",
											...
											handleInputChange({
												target: { name: "hora", value: horaStr },
											});
										}}
									/>
								)}
								...
								{(() => {
									...
									const esViajeRetornoSinDescuento =
										reservaActiva &&
									...
									const horaTerminoFormateada =
										horaTerminoServicioActivo?.toLocaleTimeString("es-CL", {
											hour: "2-digit",
											minute: "2-digit",
										});
											...
											<div className="flex items-start gap-3">
												...
												<div>
													...
													<p className="text-xs text-chocolate-600 mt-1">
														Selecciona una hora cercana al término de tu viaje
														de ida (aproximadamente {horaTerminoFormateada})
																...
																<Badge
																	...
																	onClick={() => {
																		handleInputChange({
																			target: {
																				name: "hora",
																				value: opcion.horaFormateada,
																			},
																		});
																	}}
																>
																	{opcion.horaFormateada} → {opcion.descuento}%
																	dcto
																</Badge>
															))}
														</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
									...
									<div
										...
										onClick={(e) => {
											...
											handleInputChange({
												target: { name: "idaVuelta", value: newValue },
											});
											if (!newValue)
												handleInputChange({
									...
									>
										<div className="flex flex-col flex-1 mr-3">
											<Label
												htmlFor="idaVuelta"
												className="cursor-pointer font-bold text-foreground text-sm leading-tight mb-1"
											>
												Necesito regreso
											</Label>
										...
										<div
											...
											onClick={(e) => {
												if (
													e.target.tagName === "BUTTON" ||
													e.target.closest("button") ||
													e.target.closest(".no-click-prop")
												)
													return;
										...
										>
											<div className="flex items-center justify-between w-full">
												<div className="flex items-center gap-2 flex-1 mr-3">
													<div
														className={`p-1.5 rounded-lg transition-colors ${formData.sillaInfantil ? "bg-chocolate-500 text-white" : "bg-chocolate-100 text-chocolate-600"}`}
													>
														<Baby className="h-4" />
													</div>
											...
											>
												<div className="mt-2 p-4 bg-chocolate-500/5 rounded-2xl border border-chocolate-200/30">
													<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
														<Label
															htmlFor="cantidad-sillas"
															className="text-xs font-bold text-chocolate-800 uppercase tracking-tight"
														>
															Cantidad de sillas
														</Label>
														<div className="flex items-center gap-3 p-1">
															{[...Array(configSillas.maxSillas)].map(
																(_, i) => (
																	<button
																		key={i + 1}
																		type="button"
									...
									{errorRequiereVan && (
										<div className="space-y-2">
											<p className="text-xs text-center">
												Por favor comunícate con nosotros para coordinar tu
												viaje.
											</p>
											<WhatsAppButton
					...
					{currentStep === 1 && (
						...
						>
							...
							<div className="bg-muted/30 p-4 rounded-xl border border-border space-y-3">
									...
									<div className="flex-1 min-w-0">
										...
										<p className="font-semibold text-foreground text-sm md:text-base break-words">
											{formData.origen === "Otro"
												? formData.otroOrigen
												: formData.origen}
											<span className="mx-1 md:mx-2 text-muted-foreground">
												→
											</span>
								...
								<div className="flex justify-between">
									<div>
										...
										{formData.idaVuelta && formData.fechaRegreso && (
											<p className="font-medium text-sm mt-1">
												...
												).toLocaleDateString("es-CL", { dateStyle: "long" })}
											</p>
										)}
										{formData.sillaInfantil &&
											formData.cantidadSillasInfantiles > 0 && (
												<p className="text-xs font-medium text-chocolate-700 flex items-center gap-1 mt-1">
													<Baby className="w-3 h-3" />
													{formData.cantidadSillasInfantiles}{" "}
													{formData.cantidadSillasInfantiles === 1
														? "silla infantil"
														: "sillas infantiles"}
												</p>
											)}
									...
									<div className="text-right flex flex-col items-end">
										...
										{urgencyMessage && (
											<Badge
												variant="outline"
												className="mt-1 border-muted-foreground/30 text-muted-foreground text-[10px] px-2 py-0 h-5 font-medium flex items-center gap-1 animate-in fade-in zoom-in duration-300"
											>
												{urgencyMessage.icon}
												{urgencyMessage.text}
											</Badge>
								...
								{(descuentoRetorno ||
									pricing?.descuentoRetornoUniversal > 0) && (
							...
							{formData.pasajeros &&
								...
								(() => {
									...
									const destinoInfo = destinosData.find(
										(d) => d.nombre === tramo,
									);
									if (!destinoInfo || !destinoInfo.precios?.van?.base)
										return null;
									...
									const precioSedanBase =
										baseAuto +
										adicionales * baseAuto * preciosAuto.porcentajeAdicional;
									const precioSedanSinDescuentos = formData.idaVuelta
										? precioSedanBase * 2
										: precioSedanBase;
									...
									if (formData.upgradeVan) {
										...
										const precioVan4PaxSinDescuentos = formData.idaVuelta
											? precioBaseVan * 2
											: precioBaseVan;
										const porcentajeDescuento =
											precioVan4PaxSinDescuentos > 0
												? 1 - precioActualVan / precioVan4PaxSinDescuentos
												: 0;
										precioSedanConDescuentos =
											precioSedanSinDescuentos * (1 - porcentajeDescuento);
									} else {
										...
										? precioVan4PaxSinDescuentos > 0
											? 1 -
												(pricing.totalConDescuento || 0) /
													precioVan4PaxSinDescuentos
											: 0
										: precioSedanSinDescuentos > 0
											? 1 -
												(pricing.totalConDescuento || 0) /
													precioSedanSinDescuentos
											: 0;

									// Aplicar el mismo porcentaje de descuento a la Van para 4 pax
									const precioVan4PaxConDescuentos =
										precioVan4PaxSinDescuentos * (1 - porcentajeDescuento);

									// Diferencia = Van 4 pax con descuentos - Sedán con descuentos (SIEMPRE FIJA)
									const diferenciaTotal =
										precioVan4PaxConDescuentos - precioSedanConDescuentos;
										...
										<div className="mt-6 pt-6 border-t border-border">
											...
											<div
												...
												onClick={() =>
													setFormData((prev) => ({
														...prev,
														upgradeVan: !prev.upgradeVan,
													}))
												}
											>
												{/* Checkbox visualmente grande */}
												<div
													className={`mt-0.5 w-6 h-6 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${formData.upgradeVan ? "bg-primary border-primary" : "bg-white border-input"}`}
												>
													{formData.upgradeVan && (
														<svg
												...
												>
													...
													<div className="space-y-2">
														<p className="text-xs text-muted-foreground leading-relaxed">
															<strong>Más espacio y confort:</strong> Asientos
															amplios y reclinables
															<br />
															<strong>Equipaje extra:</strong> Ideal para
															maletas grandes o compras
														</p>

														{parseInt(formData.pasajeros) === 3 && (
															<div className="bg-muted/50 border border-border rounded px-3 py-2">
																<p className="text-xs text-muted-foreground">
																	<strong>Nota:</strong> Si suman un pasajero
																	más, la Van es obligatoria por solo un pequeño
																	adicional
																</p>
															</div>
														)}
							...
							>
								...
								<Label
									htmlFor="terms"
									className="text-sm md:text-sm text-muted-foreground leading-snug cursor-pointer select-none"
								>
									Acepto los
									<Dialog>
										...
										<DialogContent onClick={(e) => e.stopPropagation()}>
											<DialogHeader>
												...
												<DialogDescription>
													Por favor, lee atentamente nuestras condiciones de
													servicio.
												</DialogDescription>
											</DialogHeader>
				...
				<div className="absolute top-0 left-0 w-full px-16 py-24 z-10 flex flex-col justify-start h-full">
					...
					>
						...
						<motion.div variants={itemVariants}>
							<h1
								className="text-6xl font-extrabold text-white mb-6 tracking-tight leading-none"
								style={{
									textShadow:
							...
							<p
								className="text-2xl text-white font-semibold border-l-4 border-primary/80 pl-4 py-1"
											...
											{richInfo.beneficios.map((b, i) => (
												...
												>
													...
													>
														...
														<b.icon
															className={cn(
																"w-5 h-5 drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]",
																b.color,
															)}
														/>
													</div>
									...
									<div className="flex items-center gap-4 pt-2 border-t border-white/10">
										<div className="flex -space-x-2">
											{[1, 2, 3].map((i) => (
												<div
													key={i}
													className="w-8 h-8 rounded-full border-2 border-black bg-gray-200 overflow-hidden flex items-center justify-center"
												>
							...
							<>
								<motion.div variants={itemVariants} className="flex gap-4">
									<div className="flex items-center gap-3 bg-[#2a4e25]/10 backdrop-blur-md rounded-xl p-4 border border-[#2a4e25]/10 flex-1">
										...
										<div>
											<p className="text-xs text-foreground/70 uppercase font-semibold">
												Distancia
											</p>
								...
								{richInfo.datoCurioso && (
									<motion.div
										variants={itemVariants}
										className="mt-6 bg-chocolate-900/40 backdrop-blur-md p-5 rounded-xl border border-chocolate-400/30"
									>

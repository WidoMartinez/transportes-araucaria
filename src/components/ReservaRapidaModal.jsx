...
export default function ReservaRapidaModal({ isOpen, onClose, promocion }) {
	...
	const getTimeOptions = (fechaSeleccionada) => {
		...
		if (promocion.hora_inicio || promocion.hora_fin) {
			options = options.filter((opt) => {
				...
				const inicio = promocion.hora_inicio
					? promocion.hora_inicio.slice(0, 5)
					: "00:00";
				const fin = promocion.hora_fin
					? promocion.hora_fin.slice(0, 5)
					: "23:59";
		...
		if (fechaSeleccionada === hoyChile) {
			...
			const anticipacionMinima =
				promocion &&
				promocion.anticipacion_minima !== undefined &&
				promocion.anticipacion_minima !== null
					? Number(promocion.anticipacion_minima)
					: 3;
	...
	const timeOptionsIda = useMemo(
		() => getTimeOptions(formData.fecha_ida),
		[formData.fecha_ida],
	);
	const timeOptionsVuelta = useMemo(
		() => getTimeOptions(formData.fecha_vuelta),
		[formData.fecha_vuelta],
	);
		...
		try {
			...
			if (paymentData.url) {
					...
					new Promise((resolve) => {
						...
						const iv = setInterval(() => {
							if (typeof window.gtag === "function") {
								...
								clearInterval(iv);
								resolve();
							} // 2s máx para no bloquear la navegación
						}, 50);
					});
				...
				if (typeof window.gtag === "function") {
					...
					console.log(
						`🚀 [ReservaRapidaModal] Lead banner disparado: reservaId=${data.reserva.id}`,
					);
				}
				...
				try {
					localStorage.setItem(
						...
						JSON.stringify({
							reservaId: String(data.reserva.id),
							amount: precioTotal,
						}),
					);
				} catch (_e) {
				...
				alert(
					`Reserva creada: ${data.reserva.codigo_reserva}. Por favor contacta a soporte para completar el pago.`,
				);
				onClose();
			}
		} catch (error) {
			...
			alert(
				error.message ||
					"Error al crear reserva. Por favor, intente nuevamente.",
			);
		} finally {
			setLoading(false);
		}
			...
			<DialogContent className="max-w-[95vw] md:max-w-2xl max-h-[95vh] overflow-y-auto p-4 md:p-6 rounded-xl">
				...
				<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="grid grid-cols-2 md:grid-cols-2 gap-y-4 gap-x-2 md:gap-4">
							<div className="flex items-start md:items-center gap-2 col-span-2 md:col-span-1">
								...
								<div className="flex-1">
									<p className="text-[10px] md:text-xs text-green-700 uppercase font-bold tracking-wider">
										Ruta
									</p>
									<p className="text-sm md:text-base font-bold text-gray-800 leading-tight">
										{origenMostrado}{" "}
										<ArrowRight className="inline h-3 w-3 md:h-4 md:w-4 mx-0.5" />{" "}
										{destinoMostrado}
									</p>
								...
								<div className="flex flex-col items-end">
									...
									{desgloseSillas > 0 && (
										<span className="text-[10px] text-green-700/80 font-medium mt-0.5">
											Incluido: +${desgloseSillas.toLocaleString("es-CL")}{" "}
											(Sillas)
										</span>
									)}
								</div>
					...
					<div className="space-y-4">
						<h3 className="font-bold text-base md:text-lg border-b pb-2">
							Tus Datos
						</h3>
										...
										{Array.from(
											{
												length: Math.max(
													1,
													(promocion.max_pasajeros || 3) -
														(promocion.min_pasajeros || 1) +
														1,
												),
											},
					...
					{promocion.permite_sillas && (
						<div className="bg-amber-50/50 p-4 rounded-xl border-2 border-amber-100 space-y-3">
							...
							<div
								className={`group flex flex-col justify-center p-4 rounded-xl border-2 transition-all cursor-pointer touch-manipulation ${
							...
							>
								<div className="flex items-center justify-between w-full">
									<div className="flex items-center gap-3">
										<div
											className={`p-2 rounded-full transition-colors ${formData.sillaInfantil ? "bg-amber-100 text-amber-600" : "bg-gray-100 text-gray-400"}`}
										>
										...
										<div>
											<Label
												...
												className="text-sm md:text-base font-bold text-gray-900 cursor-pointer block leading-tight"
											>
												Silla para niños
											</Label>
											{promocion.valor_silla > 0 && (
												<span className="text-[10px] md:text-xs text-amber-700 font-bold uppercase tracking-wide">
													+$
													{Number(promocion.valor_silla).toLocaleString(
														"es-CL",
													)}{" "}
													c/u
												</span>
							...
							<div className="p-3 md:p-4 bg-gray-50/80 rounded-xl border border-gray-200 shadow-sm space-y-3">
								...
								<div className="flex gap-2 md:gap-4">
									<div className="flex-1 space-y-1.5">
										<Label
											htmlFor="fecha_ida"
											className="text-[10px] uppercase text-gray-500 font-bold tracking-tight"
										>
											Fecha de Ida
										</Label>
										<div className="relative">
					...
					<div className="flex flex-col-reverse md:flex-row gap-3 pt-4 border-t md:justify-end sticky md:relative bottom-0 bg-white/80 md:bg-transparent backdrop-blur-sm md:backdrop-blur-none pb-1 mt-6">
						...
						>
							{loading
								? "Procesando..."
								: `Pagar $${precioTotal.toLocaleString("es-CL", { maximumFractionDigits: 0 })}`}
						</Button>
					</div>
				</form>
			</DialogContent>

...
const steps = [
        ...
        { id: 1, title: "Extras y beneficios", description: "Personaliza tu traslado" },
...
const extrasCatalog = [
        {
                ...
                description: "Recepción personalizada en sala de llegadas y asistencia con equipaje.",
                amount: 8000,
        },
        {
                id: "silla_bebe",
                name: "Silla para bebé o niño",
                description: "Instalamos silla certificada antes del viaje para menores de hasta 6 años.",
                amount: 6000,
        },
        {
                id: "mascota",
                name: "Traslado de mascota",
                description: "Espacio y protección adicional para transportar a tu mascota con seguridad.",
                amount: 5000,
        },
        {
                id: "equipaje_extra",
                name: "Equipaje adicional",
                description: "Incluye carga de equipaje voluminoso (skis, tablas o equipamiento deportivo).",
                amount: 7000,
        },
];
...
const ReservaWizard = ({
        ...
        const [paymentState, setPaymentState] = useState({ status: "idle", loading: false, error: null, receiptUrl: null });
        ...
        useEffect(() => {
                if (Number(tripData.pasajeros) > maxPasajerosSeleccionados) {
                        setTripData((prev) => ({ ...prev, pasajeros: maxPasajerosSeleccionados }));
                }
        ...
        const extrasTotal = useMemo(
                () =>
                        selectedExtras.reduce(
                                (accumulator, extra) => accumulator + (extra.amount || extra.precio || 0),
                                0
                        ),
                [selectedExtras]
        );
        ...
        const normalizedDepositRate = paymentMode === "full" ? 1 : Math.min(Math.max(depositPercentage / 100, 0.2), 1);
        ...
        const paymentAmount = paymentMode === "full" ? pricingBreakdown.total : abonoCalculado.amount;
        ...
        const isStepValid = useMemo(() => {
                if (currentStep === 0) {
                        const destinoValido =
                                (tripData.destino && tripData.destino !== "Otro") || tripData.otroDestino.trim() !== "";
        ...
        const applyCoupon = () => {
                ...
                if (!normalized) {
                        setCouponFeedback({ type: "error", message: "Ingresa un código de cupón válido." });
                        return;
                }
                const coupon = availableCoupons[normalized];
                if (!coupon) {
                        setAppliedCoupon(null);
                        setCouponFeedback({ type: "error", message: "El cupón ingresado no es válido." });
                        return;
                }
                setAppliedCoupon({ ...coupon, code: normalized });
                setCouponFeedback({ type: "success", message: `Cupón ${normalized} aplicado correctamente.` });
        };
        ...
        const resetWizard = () => {
                ...
                setDepositPercentage(Math.min(Math.max(Math.round(defaultDepositRate * 100), 20), 100));
                setSelectedPaymentMethod("mercadopago");
                setPaymentState({ status: "idle", loading: false, error: null, receiptUrl: null });
        };
        ...
        const handleMercadoPagoSubmit = async (formData) => {
                setPaymentState({ status: "processing", loading: true, error: null, receiptUrl: null });
                        ...
                        return (
                                <div className="flex flex-col items-center text-center space-y-4 py-12">
                                        ...
                                        <div className="space-y-2">
                                                <h3 className="text-2xl font-semibold">¡Pago recibido con éxito!</h3>
                                                <p className="text-muted-foreground max-w-xl">
                                                        Tu reserva ha quedado confirmada y recibirás un correo con todos los detalles y comprobantes.
                                                        Si necesitas realizar cambios, nuestro equipo estará encantado de ayudarte.
                                                </p>
                                                {paymentState.error && (
                        ...
                        return (
                                <div className="space-y-6">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div className="space-y-2">
                                                        ...
                                                        <Input
                                                                ...
                                                                onChange={(event) => handleTripChange("origen", event.target.value)}
                                                        />
                                                </div>
                                                <div className="space-y-2">
                                                        ...
                                                        <select
                                                                ...
                                                                onChange={(event) => handleTripChange("destino", event.target.value)}
                                                        >
                                                ...
                                                {tripData.destino === "Otro" && (
                                                        <div className="space-y-2 md:col-span-2">
                                                                ...
                                                                <Input
                                                                        ...
                                                                        onChange={(event) => handleTripChange("otroDestino", event.target.value)}
                                                                />
                                                        </div>
                                                )}
                                                <div className="space-y-2">
                                                        ...
                                                        <select
                                                                ...
                                                                onChange={(event) => handleTripChange("pasajeros", Number(event.target.value))}
                                                        >
                                                                {Array.from({ length: maxPasajerosSeleccionados }, (_, index) => index + 1).map((num) => (
                                                ...
                                                <div className="space-y-2">
                                                        ...
                                                        <Input
                                                                ...
                                                                onChange={(event) => handleTripChange("fecha", event.target.value)}
                                                        />
                                                </div>
                                                <div className="space-y-2">
                                                        ...
                                                        <Input
                                                                ...
                                                                type="time"
                                                                value={tripData.hora}
                                                                onChange={(event) => handleTripChange("hora", event.target.value)}
                                                        />
                                                </div>
                                                <div className="space-y-2">
                                                        ...
                                                        <Input
                                                                ...
                                                                onChange={(event) => handleTripChange("nombre", event.target.value)}
                                                        />
                                                </div>
                                                <div className="space-y-2">
                                                        ...
                                                        <Input
                                                                ...
                                                                onChange={(event) => handleTripChange("telefono", event.target.value)}
                                                        />
                                                        {tripData.telefono && !telefonoRegex.test(tripData.telefono.trim()) && (
                                                                <p className="text-xs text-red-500">Ingresa un número chileno válido.</p>
                                                        )}
                                                </div>
                                                <div className="space-y-2">
                                                        ...
                                                        <Input
                                                                ...
                                                                onChange={(event) => handleTripChange("email", event.target.value)}
                                                        />
                                                </div>
                                                <div className="space-y-2 md:col-span-2">
                                                        ...
                                                        <Textarea
                                                                ...
                                                                onChange={(event) => handleTripChange("notas", event.target.value)}
                                                        />
                                                </div>
                                        </div>
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 bg-muted/40 rounded-lg p-4">
                                                <div className="space-y-2">
                                                        ...
                                                        {cotizacion.precio ? (
                                                                <p className="text-muted-foreground text-sm">
                                                                        Tarifa calculada automáticamente según destino y número de pasajeros.
                                                                </p>
                                                        ) : (
                                                                <p className="text-muted-foreground text-sm">
                                                                        Ingresa una tarifa base referencial para continuar con el proceso.
                                                                </p>
                                                        )}
                                                ...
                                                <div className="space-y-2">
                                                        <Label htmlFor="tarifa-manual">Tarifa base manual (opcional)</Label>
                                                        <Input
                                                                id="tarifa-manual"
                                                                type="number"
                                                                min="0"
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <div className="space-y-2">
                                                        ...
                                                        <select
                                                                ...
                                                                onChange={(event) => handleClubBenefitChange(event.target.value)}
                                                        >
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                                <div className="space-y-3 p-4 border rounded-lg bg-white shadow-sm">
                                                        ...
                                                        <div className="flex justify-between text-sm text-emerald-700">
                                                                ...
                                                                <span>-{formatoMonedaCLP(pricingBreakdown.totalDiscounts)}</span>
                                                        </div>
                                                        <div className="flex justify-between text-sm">
                                                                <span>Impuestos</span>
                                                        ...
                                                        {selectedExtras.length > 0 && (
                                                                <div className="pt-3 border-t text-sm space-y-1">
                                                                        <p className="font-semibold text-foreground">Extras incluidos</p>
                                                        ...
                                                        {(pricingBreakdown.onlineDiscountValue > 0 ||
                                                                ...
                                                                <div className="pt-3 border-t text-sm space-y-1">
                                                                        <p className="font-semibold text-foreground">Beneficios aplicados</p>
                                                                        <ul className="list-disc pl-4 text-emerald-700 space-y-1">
                                                                                {pricingBreakdown.onlineDiscountValue > 0 && (
                                                                                        <li>
                                                                                                Descuento online {Math.round((pricingBreakdown.onlineDiscountRate || 0) * 100)}%:
                                                ...
                                                <div className="space-y-4 p-4 border rounded-lg bg-white shadow-sm">
                                                        ...
                                                        <RadioGroup value={paymentMode} onValueChange={setPaymentMode} className="space-y-3">
                                                                <div className={`border rounded-md p-3 ${paymentMode === "deposit" ? "border-primary" : "border-muted"}`}>
                                                                        <div className="flex items-start gap-2">
                                                                                ...
                                                                                <Label htmlFor="payment-deposit" className="flex-1">
                                                                                        ...
                                                                                        <span className="text-sm text-muted-foreground">
                                                                                                Reserva tu traslado abonando un porcentaje y paga el saldo al conductor.
                                                                                        </span>
                                                                                </Label>
                                                                        </div>
                                                                        {paymentMode === "deposit" && (
                                                                                <div className="mt-3 space-y-2">
                                                                                        ...
                                                                                        <div className="text-sm space-y-1">
                                                                                                ...
                                                                                                <p>Saldo pendiente: {formatoMonedaCLP(saldoPendiente)}</p>
                                                                                        </div>
                                                                                </div>
                                                                        )}
                                                                </div>
                                                                <div className={`border rounded-md p-3 ${paymentMode === "full" ? "border-primary" : "border-muted"}`}>
                                                                        <div className="flex items-start gap-2">
                                                                                <RadioGroupItem value="full" id="payment-full" />
                                                                                <Label htmlFor="payment-full" className="flex-1">
                                                                                        <span className="font-semibold block">Pagar el total ahora</span>
                                                                                        <span className="text-sm text-muted-foreground">
                                                                                                Cancela el 100% y recibe la confirmación inmediata sin saldos pendientes.
                                                                                        </span>
                                                                                </Label>
                                                                        </div>
                                                                </div>
                                                ...
                                                {selectedPaymentMethod === "mercadopago" ? (
                                                        publicKey ? (
                                                                <div className="space-y-3">
                                                                        ...
                                                                        <p className="text-xs text-muted-foreground">
                                                                                Operamos con tokenización y autenticación 3-D Secure de Mercado Pago para resguardar tus datos.
                                                                        </p>
                                                                </div>
                                                        ) : (
                                                                <p className="text-sm text-amber-600">
                                                                        Configura la clave pública de Mercado Pago (VITE_MP_PUBLIC_KEY) para habilitar el pago en línea.
                                                                </p>
                                                        )
                                                ) : (
                                                        <p className="text-sm text-muted-foreground">
                                ...
                                <Card className="shadow-xl border border-muted/40">
                                        <CardHeader className="space-y-4">
                                                <CardTitle className="text-3xl font-semibold text-foreground flex items-center gap-3">
                                                        <Sparkles className="h-7 w-7 text-primary" /> Reserva digital asistida
                                                </CardTitle>
                                                <CardDescription className="text-muted-foreground">
                                                        Diseñamos un asistente paso a paso para que puedas reservar, sumar extras y abonar en línea sin salir del sitio.
                                                </CardDescription>
                                                        ...
                                                        {steps.map((step, index) => (
                                                                <div
                                                                        ...
                                                                        className={`flex items-center gap-3 px-3 py-2 rounded-full border ${
                                                                                currentStep === index ? "bg-primary/10 border-primary" : "bg-muted border-muted"
                                                                        }`}
                                                                >
                                                                        <Badge variant={currentStep === index ? "default" : "secondary"} className="rounded-full">
                                                                                {index + 1}
                                                                        </Badge>
                                                        ...
                                                        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
                                                                ...
                                                                {currentStep < steps.length - 1 ? (
                                                                        ...
                                                                        <p className="text-xs text-muted-foreground text-center md:text-right">
                                                                                El pago se procesa con tokenización y 3-D Secure de Mercado Pago para tu tranquilidad.
                                                                        </p>
                                                                )}
                                                        </div>
                                                )}

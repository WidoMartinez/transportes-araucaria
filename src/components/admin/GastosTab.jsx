...
export function GastosTab({ reserva, onGastosUpdate }) {
	...
	const createDraftGasto = useCallback(
		(initialTipoGasto = "") => {
			const defaultConductorId =
				reserva?.conductorId != null
					? reserva.conductorId.toString()
					: "";
			const defaultVehiculoId =
				reserva?.vehiculoId != null
					? reserva.vehiculoId.toString()
					: "";
			...
			if (
				...
				const base = parseFloat(reserva.totalConDescuento) || 0;
	...
	// Resetear drafts cuando cambia la reserva o se abre el dialogo
	useEffect(() => {
		if (showBulkDialog && reserva?.id) {
			const initialDrafts = TIPOS_GASTO.map((tipo) => createDraftGasto(tipo.value));
			setDraftGastos(initialDrafts);
		}
	}, [showBulkDialog, reserva?.id, createDraftGasto]);
	...
	const handleDraftTipoChange = (id, value) => {
		...
		if (value === "comision_flow" && reserva) {
			const porcentaje = 3.19;
			const base = parseFloat(reserva.totalConDescuento || 0);
	...
	const handleSaveBulkGastos = async () => {
		...
		if (validDrafts.length === 0) {
			alert("Agrega al menos un gasto válido");
			return;
		}
	...
	const totalReservaNeta = useMemo(() => {
		const valor = reserva?.totalConDescuento ?? 0;
	...
	const conductorAsignadoReserva =
		reserva?.conductorId != null
			? conductores.find(
					(conductor) => conductor.id === Number(reserva.conductorId)
			  )
			: null;
	...
	const vehiculoAsignadoReserva =
		reserva?.vehiculoId != null
			? vehiculos.find(
					(vehiculo) => vehiculo.id === Number(reserva.vehiculoId)
			  )
			: null;
				...
				<CardHeader className="flex flex-row items-center justify-between">
					<div>
						...
						<p className="text-sm text-muted-foreground mt-1">
							{reserva.codigoReserva} - {reserva.nombre}
						</p>
					</div>
								...
								<TableBody>
									{gastos.map((gasto) => (
										<TableRow key={gasto.id}>
											...
											<TableCell>
												{gasto.vehiculo ? `${gasto.vehiculo.patente}` : "-"}
											</TableCell>
			...
			<Dialog open={showBulkDialog} onOpenChange={setShowBulkDialog}>
				<DialogContent className="max-w-4xl">
					<DialogHeader>
						...
						<DialogDescription>
							Agrega varios gastos y guárdalos en un solo paso.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4">
						...
						{draftGastos.length === 0 ? (
							<div className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">
								No hay gastos en preparación. Usa "Agregar gasto" para comenzar.
							</div>
						) : (
									...
									>
										...
										{draft.tipoGasto === "comision_flow" && (
											<div className="rounded-md bg-blue-50 p-3 text-xs text-blue-800">
												...
												El monto se calcula automáticamente como el 3.19% del
												total de la reserva.
											</div>
										)}
						...
						<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
							...
							<div className="text-right text-sm">
								<p>
									Nuevos gastos:{" "}
									<strong>${totalDraftMonto.toLocaleString("es-CL")}</strong>
								</p>
								<p className="text-muted-foreground">
									Se sumará al total actual de $
									{totalGastos.toLocaleString("es-CL")}.
								</p>
							</div>
						</div>

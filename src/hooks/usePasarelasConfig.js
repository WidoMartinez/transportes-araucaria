...
export function usePasarelasConfig() {
  ...
  const cargar = async (forzar = false) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${getBackendUrl()}/api/configuracion/pasarelas-pago`);
      ...
      // Guardar en caché
      try {
        sessionStorage.setItem(CACHE_KEY, JSON.stringify({ data: datos, ts: Date.now() }));
      } catch {
        // sessionStorage puede no estar disponible
      }
    } catch (err) {
      console.warn("No se pudo cargar configuración de pasarelas:", err.message);
